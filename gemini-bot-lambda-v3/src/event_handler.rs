...
use crate::libs::MedicalTerms;
use serde::{Deserialize, Serialize};

#[derive(Debug, Serialize, Deserialize)]
struct MedicalDummie {
    medical_terms: Vec<MedicalTerms>,
}
...
pub(crate)async fn function_handler(event: LambdaEvent<EventBridgeEvent>) -> Result<(), Error> {
    ...
    let medical_info = "Most Responsible Diagnosis: COPD Exacerbation  Active Issues Managed in Hospital";

    match mongodb_update(user_id, medical_info).await {
