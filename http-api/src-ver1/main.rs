...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if let Some(url) = response_json.get("output").and_then(|u| u.as_str()) {
        // Download the file
        let mut file_response = client.get(url).send()?;
        
        // Get the current date
        let date_now = Utc::now().format("%Y-%m-%dT%H-%M-%S").to_string();
        let file_name = format!("output-{}.png", date_now);
        
        let mut out = File::create(&file_name)?;
        copy(&mut file_response, &mut out)?;
