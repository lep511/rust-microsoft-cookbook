...
async fn example(cluster_endpoint: String, password_token: String) -> anyhow::Result<()> {
    // Setup connections
...
pub(crate) async fn function_handler(event: Request) -> Result<Response<Body>, Error> {
    ...
    let cluster_endpoint = String::from("kaabtu6ozb2mhkawb7malic4mm.dsql.us-east-1.on.aws");

    let token = generate_token(cluster_endpoint.clone(), String::from("us-east-1")).await;
    let _ = example(cluster_endpoint, token).await;
