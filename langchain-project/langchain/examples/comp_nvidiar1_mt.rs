...
pub async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create a JoinSet to manage spawned tasks
    let mut join_set = JoinSet::new();
    
    // Spawn a task for each metric evaluation
    ...
    while let Some(result) = join_set.join_next().await {
        match result {
            Ok((metric_name, output)) => {
                println!("=== {} Evaluation ===", metric_name);
                println!("{}", output);
                println!("\n");
            },
