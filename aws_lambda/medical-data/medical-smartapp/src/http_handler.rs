...
pub(crate) async fn function_handler(event: Request) -> Result<Response<Body>, Error> {
    ...
    // Extract version and resource_path from the request context
    let version = match &request {
        RequestContext::ApiGatewayV1(ctx) => {
            ctx.stage.clone().unwrap_or_else(|| "unknown".to_string())
        },
        _ => "unknown".to_string(),
    };
    ...
    let resource = match &request {
        RequestContext::ApiGatewayV1(ctx) => {
            ctx.resource_path.clone().unwrap_or_else(|| "unknown".to_string())
        },
        _ => "unknown".to_string(),
    };
